(() => {
  ...
  const defaults = {
    version:14,appName:"Berna",mikiName:"Miki",coins:125,xp:0,totalPomodoros:0,
    tasks:[],habits:[],subjects:[],studyBlocks:[],journal:{},friends:[],sessions:[],
    ...
    pet:{hunger:82,happy:82,energy:82,lastDecayDate:""},
    room:{season:"spring",weather:"sunny"},
  ...
  function applyPetDecay(){
    ...
    if(days){state.pet.hunger=clamp(state.pet.hunger-days*8);state.pet.happy=clamp(state.pet.happy-days*5);state.pet.energy=clamp(state.pet.energy-days*4);state.pet.lastDecayDate=todayKey();}
  }
  ...
  const refs={
    ...
    hungerBar:$("#hungerBar"),happyBar:$("#happyBar"),energyBar:$("#energyBar"),hungerText:$("#hungerText"),happyText:$("#happyText"),energyText:$("#energyText"),petBubble:$("#petBubble"),
  ...
  function questData(){const t=todayKey();return [{id:"tasks",name:"2 görev tamamla",current:Math.min(2,tasksOn(t).length),goal:2},{id:"focus",name:"1 Pomodoro tamamla",current:Math.min(1,sessionsOn(t).length),goal:1},{id:"habits",name:"3 alışkanlık işaretle",current:Math.min(3,habitChecksOn(t)),goal:3}];}
  ...
  function renderTimer(){
    ...
    const total=modeSeconds(),deg=Math.min(360,Math.round((total-state.timer.remaining)/total*360));refs.timerRing.style.background=`conic-gradient(var(--pink) ${deg}deg,#f3e7e1 ${deg}deg)`;document.title=state.timer.running?`${refs.timerDisplay.textContent} · ${state.appName}`:`${state.appName} V14 · Miki Edition`;
  }
  ...
  function renderPet(){
    [["hunger",refs.hungerBar,refs.hungerText],["happy",refs.happyBar,refs.happyText],["energy",refs.energyBar,refs.energyText]].forEach(([k,bar,text])=>{bar.style.width=`${state.pet[k]}%`;text.textContent=Math.round(state.pet[k]);});
    refs.petBubble.textContent=state.pet.hunger<30?"Karnım biraz acıktı…":state.pet.energy<25?"Biraz uyusam iyi olacak.":state.pet.happy<35?"Birlikte oynayalım mı?":"Yanındayım. Küçük bir adım atalım!";
  ...
  function care(action){
    ...
    state.pet.lastDecayDate=todayKey();save();renderAll();
  }
